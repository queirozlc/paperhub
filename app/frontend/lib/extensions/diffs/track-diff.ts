...
export const TrackChangeExtension = Extension.create<{
  ...
  onTransaction: (props: { editor: Editor; transaction: Transaction }) => {
    ...
    if (syncMeta && syncMeta.isChangeOrigin) {
      if (LOG_ENABLED) {
        console.log('sync from origin', syncMeta)
      }
      return
    }
    // has no real step
    if (!transaction.steps.length) {
      if (LOG_ENABLED) {
        console.log('none content change')
      }
      return
    }
    ...
    if (LOG_ENABLED) {
      console.log('currentNewPos', currentNewPos)
    }
    ...
    allSteps.forEach((step: Step, index: number) => {
      if (step instanceof ReplaceStep) {
        ...
        if (step.from !== step.to && trackChangeEnabled) {
          if (LOG_ENABLED) {
            console.log('find content to readd', step)
          }
    ...
    if (trackChangeEnabled) {
      ...
      if (LOG_ENABLED) {
        console.log('update cursor', finalNewPos)
      }
